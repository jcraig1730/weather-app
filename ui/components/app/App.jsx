...
import { ipGeoLocation } from '../../../keys.js';
...
export default function App() {
  ...
  const getUser = () => new Promise((resolve, reject) => {
    Axios.get(`http://localhost:3002/api/users/${user.sub}`)
  ...
  const findIpLocation = async (ip) => {
    const { data } = await Axios.get(`https://api.ipgeolocation.io/ipgeo?apiKey=${ipGeoLocation}&ip=${ip}&fields=geo`);
    return data.zipcode;
  };

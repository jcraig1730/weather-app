...
function WeatherMap(props) {
  ...
  const { GOOGLE_API_KEY, WEATHER_API_KEY, ZIP_API_URL } = process.env;

  const handleMarkerClick = e => {
    const { lat, lng } = e.position;

    Axios.get(
      // `http://${ZIP_API_URL}/api/zipbycoords/${lat}/${lng}`
  ...
  const getWeather = (northLat, eastLng, southLat, westLng) =>
    new Promise((resolve, reject) => {
      Axios.get(
        `http://api.openweathermap.org/data/2.5/box/city?bbox=${westLng},${northLat},${eastLng},${southLat},${zoom}&cluster=yes&format=json&APPID=${WEATHER_API_KEY}`
      ).then(data => {
  ...
  useEffect(() => {
    ...
    if (zip) {
      Axios.get(`http://${ZIP_API_URL}/api/coordsbyzip/${zip}`)
        .then(({ data }) => {
          const center = data[0];
...
export default GoogleApiWrapper({ apiKey: 10 })(WeatherMap);
